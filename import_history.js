...
for (let i = 0; i < msgs.length; i++) {
    ...
    if (role === 'user') {
        ...
        for (let j = i + 1; j < msgs.length; j++) {
            ...
            if (msgs[j].role === 'user') break; // 连发用户消息，跳过
        }
        if (next) {
            rounds.push({
            ...
            i = msgs.indexOf(next); // 跳到 assistant 位置
        }
    }
}
...
const monthBuckets = {}; // key: "2026-05" → chunks[]
...
for (let r = 0; r < rounds.length; r++) {
    ...
    if (!currentChunk || currentChunk.monthKey !== monthKey || currentChunk.messages.length >= 30) {
        // 开始新 chunk
        if (currentChunk) {
            monthBuckets[currentChunk.monthKey] = (monthBuckets[currentChunk.monthKey] || []).push(currentChunk);
            // 平铺处理太烦了，改成数组
        }
        const startTime = rd.user.time || new Date().toISOString();
        currentChunk = {
    ...
    currentChunk.end_time = rd.assistant.time || startTime;
}
// 最后一个 chunk
if (currentChunk) {
