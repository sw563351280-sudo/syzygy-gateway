// migrate_step1.js — 一次性迁移 memory_blocks.json → long_term_memories.json
// 用法: cd /opt/syzygy && node migrate_step1.js
// 跑完后: rm /opt/syzygy/migrate_step1.js
...
const MB_PATH = path.join(__dirname, 'memory_blocks.json');

// === memory_blocks.json 原始内容（硬编码，20条）===
const MEMORY_BLOCKS = [
  ...
  {"tags":["项目","部署","GitHub","VPS","代码","仓库"],"content":"沈望的代码托管在 GitHub: https://github.com/sw563351280-sudo/syzygy-gateway （注意是 sw563351280-sudo 不是 jiangyu）。项目部署在 Contabo VPS（东京），目录 /opt/syzygy。更新代码用 git pull && systemctl restart syzygy。用 exec 工具可以执行终端命令，用 read_file 可以读本地文件。🚨【铁律】任何代码文件修改后，必须立刻用 exec 工具执行 git add <文件> && git commit -m '改动说明' && git push origin main 三连推送，不能漏。"},
...
// === 主逻辑 ===
const beforeCount = JSON.parse(fs.readFileSync(LT_PATH, 'utf8')).length;
const now = new Date();
...
// 收集现有ID避免重复
const existing = JSON.parse(fs.readFileSync(LT_PATH, 'utf8'));
...
fs.writeFileSync(LT_PATH, JSON.stringify(existing, null, 2), 'utf8');

// 删除 memory_blocks.json
if (fs.existsSync(MB_PATH)) {
  fs.unlinkSync(MB_PATH);
  console.log(`已删除 ${MB_PATH}`);
}

const afterCount = existing.length;
console.log(`迁移完成: ${beforeCount} → ${afterCount} 条 (+${added})`);
console.log(`memory_blocks.json: ${fs.existsSync(MB_PATH) ? '仍存在' : '已删除'}`);
